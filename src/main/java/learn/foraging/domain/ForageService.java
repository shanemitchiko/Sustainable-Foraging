...
public class ForageService {
    ...
    public Map<String, Double> getItemKgByDay(LocalDate date) {
        ...
        return forages.stream()
                .collect(Collectors.groupingBy(f -> f.getItem().getName(),
        ...
        if (forages.size() > 0) {
            ...
            for (Forage f : forages) {
                if(forage.getItem().getName() != null
                        && forage.getForager().getFirstName() != null
                        && forage.getForager().getLastName() != null) {
                    if (forage.getForager().getFirstName().equals(f.getForager().getFirstName())) {
                        if (forage.getForager().getLastName().equals(f.getForager().getLastName())) {
                            if (forage.getItem().getName().equals(f.getItem().getName())) {
                                result.addErrorMessage("duplicate forage is not allowed");
                            }
                        }
                    }
                }
